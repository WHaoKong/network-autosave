  ...
  <div class="tasks-view">
    ...
    <div class="toolbar">
      <div class="toolbar-left">
        ...
        <el-select v-model="statusFilter" placeholder="状态筛" style="width: 120px">
        ...
        <el-select v-model="categoryFilter" placeholder="分类筛" style="width: 140px">
        ...
        <el-tooltip content="列设" placement="top">
    ...
    <div class="task-list-container">
      ...
      >
        ...
        <el-table-column label="排序" class-name="col-drag" align="center">
          <template #default>
            <div
              ...
              :title="canDragSort ? '拖拽调整顺序' : '请先切回正序并清空筛选后再拖拽排'"
            >
        ...
        <el-table-column label="网盘类型" class-name="col-provider" v-if="columnVisible.provider">
          <template #default="{ row }">
            <el-tag size="small" :type="row.provider === 'quark' ? 'success' : 'primary'">
              {{ displayProviderName(row.provider) }}
            </el-tag>
            <div v-if="row.account" class="task-account">{{ row.account }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="分享链接" class-name="col-share" v-if="columnVisible.shareLink">
          <template #default="{ row }">
            <div class="share-link-container">
              ...
              <span v-else class="no-share">未生成分享链</span>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="status" label="状" class-name="col-status" v-if="columnVisible.status">
          <template #default="{ row }">
        ...
        <el-table-column label="操作" class-name="col-actions">
          <template #default="{ row }">
            <el-button-group size="small">
              ...
              <el-button
                @click="shareTask(row.order - 1)"
                :disabled="row.provider === 'quark'"
                :title="row.provider === 'quark' ? '夸克任务暂不支持生成分享链接' : '分享任务'"
              >
        ...
        >
          <div class="task-card-body">
            ...
            <div class="card-content">
              <div class="content-row">
                <span class="label">网盘类型:</span>
                <span class="value">{{ displayProviderName(task.provider) }}</span>
              </div>

              <div class="content-row" v-if="task.account">
                <span class="label">使用账号:</span>
              ...
              <div class="content-row" v-if="task.cron || task.regex_pattern || task.regex_replace">
                ...
                <div class="advanced-tags">
                  ...
                  <el-tag v-if="task.regex_replace" size="small" type="success">重命</el-tag>
                </div>
              </div>
          ...
          <div class="card-actions">
            <button 
              ...
              :title="task.status === 'running' ? '执行...' : '执行任务'"
            >
              <el-icon><VideoPlay /></el-icon>
            </button>
            ...
            <button 
              ...
              :disabled="task.provider === 'quark'"
              :title="task.provider === 'quark' ? '夸克任务暂不支持生成分享链接' : '分享任务'"
            >
              <el-icon><Share /></el-icon>
            </button>
    ...
    >
      <div class="column-settings">
        ...
        <el-checkbox v-model="columnVisible.provider">网盘类型</el-checkbox>
        ...
        <el-checkbox v-model="columnVisible.status">状</el-checkbox>
...
const executeTask = async (taskId: number) => {
  ...
  if (!task) {
    ElMessage.error('任务不存')
    return
  }
  
  // 检查任务是否正在运
  if (task.status === 'running') {
    ElMessage.warning('任务正在执行')
    return
  }
...
const initSortable = async () => {
  ...
  sortableInstance = Sortable.create(el, {
    ...
    onEnd: async (event) => {
      ...
      if (!canDragSort.value) {
        ElMessage.warning('请切回正序并清空筛选后再拖拽排')
        await fetchTasks()
        tableKey.value++
        return
      }
