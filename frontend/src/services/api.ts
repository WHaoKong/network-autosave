...
export class ApiService {
  ...
  async getShareInfo(url: string, pwd?: string, provider?: NetdiskProvider, account?: string): Promise<ApiResponse<any>> {
    return httpClient.post('/api/share/info', { url, pwd, provider, account })
  }

  // 移除parseShareUrl，使用现有的getShareInfo接口获取文件
  ...
  async switchUser(username: string, provider?: NetdiskProvider): Promise<ApiResponse<any>> {
    return httpClient.post('/api/user/switch', { username, provider })
  }

  async deleteUser(username: string, provider?: NetdiskProvider): Promise<ApiResponse<void>> {
    return httpClient.post('/api/user/delete', { username, provider })
  }
