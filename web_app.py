...
from storage import BaiduStorage, SUPPORTED_PROVIDERS
...
# Docker Hub 信息
DOCKER_HUB_RSS = 'https://rsshub.rssforever.com/dockerhub/tag/WHaoKong/network-autosave'
# 备用 Docker Hub RSS 
DOCKER_HUB_RSS_ALT = 'https://rss.kuaisouxia.com/dockerhub/tag/WHaoKong/network-autosave'
# 1ms.run API 
...
log_level = "DEBUG"  # 使用DEBUG级别，可以看到所有日
...
logger.add(sys.stdout, 
          ...
          filter=filter_polling_requests)  # 添加轮询过滤

# 添加文件输出 (内容与控制台输出相同，但没有颜色标记，且不过滤轮询请)
logger.add("log/web_app_{time:YYYY-MM-DD}.log", 
          rotation="00:00",  # 每天零点创建新文
...
def get_server_port():
    ...
    try:
        ...
        logger.warning(f"无效的服务端口配: {raw_port}，回退 5000")
        return 5000
...
def get_task_status(task_id):
    ...
    if not storage:
        return jsonify({'success': False, 'message': '存储未初始化'})
    tasks = storage.list_tasks()
    #  order 排序，确 task_id 对应正确的任
    tasks.sort(key=lambda x: x.get('order', float('inf')))
    if 0 <= task_id < len(tasks):
        return jsonify({'success': True, 'status': tasks[task_id]})
    return jsonify({'success': False, 'message': '任务不存'})
...
def add_task():
    ...
    provider = data.get('provider', 'baidu').strip().lower()
    account = data.get('account', '').strip()
    if provider not in SUPPORTED_PROVIDERS:
        return jsonify({'success': False, 'message': '不支持的网盘类型'})
    
    if not url or not save_dir:
        return jsonify({'success': False, 'message': '分享链接和保存目录不能为'})
    ...
    try:
        ...
        if storage.add_task(url, save_dir, pwd, name, cron, category, regex_pattern, regex_replace, provider, account):
...
def update_task():
    ...
    # 查找对应order的任
    task_order = task_id + 1  # task_id 是从0开始的索引， order 是从1开始的
    task = None
    for t in tasks:
    ...
    if not task:
        return jsonify({'success': False, 'message': f'未找到任(order={task_order})'})
    
    # 获取新的URL和密
    new_url = data.get('url', '').strip()
    new_pwd = data.get('pwd', '').strip()
    provider = data.get('provider', task.get('provider', 'baidu')).strip().lower()
    account = data.get('account', task.get('account') or '').strip()
    if provider not in SUPPORTED_PROVIDERS:
        return jsonify({'success': False, 'message': '不支持的网盘类型'})

    if '#' in new_url:
        base_url, fragment = new_url.split('#', 1)
        if not new_pwd and fragment and not fragment.startswith('/'):
            new_pwd = fragment.strip()
        new_url = base_url.strip()
    if '?pwd=' in new_url:
        new_url, extracted_pwd = new_url.split('?pwd=', 1)
        new_url = new_url.strip()
        if not new_pwd:
            new_pwd = extracted_pwd.strip()
    ...
    update_data = {
        ...
        'status': task.get('status', 'normal'),  # 保持原有的状
        'message': task.get('message', ''),  # 保持原有的消
    ...
    try:
        # 更新任务
        success, error_msg = storage.update_task_by_order(task_order, update_data)
        if not success:
            return jsonify({'success': False, 'message': error_msg or '更新任务失败'})
...
def get_share_info():
    ...
    try:
        ...
        # 获取分享文件信息
        result = storage.get_share_folder_name(url, pwd, provider, account)
...
def move_task():
    ...
    try:
        ...
        if not (0 <= task_id < len(tasks)) or not (0 <= new_index < len(tasks)):
            return jsonify({'success': False, 'message': '任务ID或位置无'})
        ...
        return jsonify({'success': True, 'message': '任务位置已更'})
...
def _resolve_request_task(task_ref=None, task_id=None, tasks=None):
    """ task_uid/order/url 解析请求中的任务，兼容旧 task_id"""
    if not storage:
        return None
...
def execute_task():
    ...
    if not task:
        if task_uid:
            return jsonify({'success': False, 'message': f'未找到任(task_uid={task_uid})'})
    ...
    if not task_order:
        return jsonify({'success': False, 'message': f'任务order不存(task_uid={task_uid or task_id})'})
    ...
    def execute_task_async():
        ...
        try:
            ...
            if not latest_task:
                ...
                _publish_task_status(task_order, task_uid)
                _append_task_log(task_order, '任务已不存在，执行失', level='ERROR', task_uid=task_uid)
            ...
            if result.get('success'):
                ...
                if transferred_files:
                    ...
                    storage.update_task_status_by_order(task_order, 'normal', '没有新文件需要转')
                    _publish_task_status(task_order, task_uid)
                    _append_task_log(task_order, '没有新文件需要转', task_uid=task_uid)
                    _publish_task_completed(task_order, task_uid)
            else:
...
def get_users():
    ...
    for user in users:
        quota = storage.get_user_quota(user.get('provider', 'baidu'), user.get('username'))
...
def add_user():
    ...
    if provider == 'quark':
        success = storage.quark_storage.add_user_from_cookies(cookies, username)
    elif provider == 'uc':
        success = storage.uc_storage.add_user_from_cookies(cookies, username)
    elif provider == 'aliyun':
        success = storage.aliyun_storage.add_generic_user_from_cookies(provider, cookies, username) if hasattr(storage.aliyun_storage, 'add_generic_user_from_cookies') else storage.add_generic_user_from_cookies(provider, cookies, username)
    elif provider == 'baidu':
        success = storage.add_user_from_cookies(cookies, username)
    else:
        success = storage.add_generic_user_from_cookies(provider, cookies, username)
...
def switch_user():
    ...
    if not username:
        return jsonify({'success': False, 'message': '用户名不能为'})
        
    try:
        if provider == 'quark':
            switched = storage.quark_storage.switch_user(username)
            user = storage.quark_storage.get_user(username)
        elif provider == 'uc':
            switched = storage.uc_storage.switch_user(username)
            user = storage.uc_storage.get_user(username)
        elif provider == 'aliyun':
            switched = storage.switch_provider_user(provider, username)
            user = storage.get_provider_user(provider, username)
        elif provider == 'baidu':
            switched = storage.switch_user(username)
        ...
        if switched:
            # 获取完整的用户信
            if not user:
                return jsonify({'success': False, 'message': f'用户 {username} 不存'})
            ...
            try:
                if storage and hasattr(storage, 'get_user_info'):
                    ...
                    if user_info and 'quota' in user_info:
                        ...
                        logger.info(f"已切换到用户: {username}，网盘总空: {total_gb}GB, 已使: {used_gb}GB")
                ...
                logger.error(f"切换用户后获取配额信息失: {str(e)}")
...
def delete_user():
    ...
    current_user = storage.config.get(provider, {}).get('current_user')
    if provider == 'baidu' and current_user == username:
        return jsonify({'success': False, 'message': '不能删除当前使用的用'})

    if provider == 'quark':
        success = storage.quark_storage.remove_user(username)
    elif provider == 'uc':
        success = storage.uc_storage.remove_user(username)
    elif provider == 'aliyun':
        success = storage.remove_provider_user(provider, username)
    elif provider == 'baidu':
        success = storage.remove_user(username)
    else:
        success = storage.remove_provider_user(provider, username)
...
def update_user():
    ...
    if provider == 'uc':
        ...
        if storage.uc_storage.update_user(username, cookies):
            return jsonify({'success': True, 'message': '用户更新成功'})
        return jsonify({'success': False, 'message': '用户更新失败'})
    if provider == 'aliyun':
        if original_username != username:
            return jsonify({'success': False, 'message': '暂不支持重命名该网盘用户'})
        if storage.update_provider_user(provider, username, cookies):
            return jsonify({'success': True, 'message': '用户更新成功'})
        return jsonify({'success': False, 'message': '用户更新失败'})
    if provider != 'baidu':
        if original_username != username:
    ...
    if original_username != username:
        ...
        if username in storage.config['baidu']['users']:
            return jsonify({'success': False, 'message': f'用户 {username} 已存'})
        
        # 获取原用户信
        user_info = storage.get_user(original_username)
        if not user_info:
            return jsonify({'success': False, 'message': f'用户 {original_username} 不存'})
...
def get_user_cookies(username):
    """获取用户cookies"""
    provider = request.args.get('provider', 'baidu')
    if provider == 'quark':
        user_info = storage.quark_storage.get_user(username)
    elif provider == 'uc':
        user_info = storage.uc_storage.get_user(username)
    elif provider == 'aliyun':
        user_info = storage.get_provider_user(provider, username)
    elif provider == 'baidu':
...
def test_notify():
    ...
    try:
        ...
        if notify_config and notify_config.get('enabled'):
            ...
            return jsonify({'success': True, 'message': '测试通知已发'})
        else:
            return jsonify({'success': False, 'message': '通知功能未启用'})
    except Exception as e:
...
def execute_all_tasks():
    ...
    for selected_task in selected_tasks:
        ...
        if not task:
            logger.warning(f"批量执行时任务已不存: {selected_task}")
        ...
        try:
            ...
            if result.get('success'):
                if result.get('skipped'):
                    ...
                    storage.update_task_status_by_order(task_order, 'skipped', '没有新文件需要转')
                else:
                    transferred_files = result.get('transferred_files', [])
                    if transferred_files:
    ...
    return jsonify({
        'success': True,
        'message': f'批量执行完成，成: {len(results["success"])}，跳: {len(results["skipped"])}，失: {len(results["failed"])}',
        'results': results
    })
...
def delete_notify_field():
    ...
    if not field_deleted:
        return jsonify({'success': False, 'message': f'未找到字: {field_name}'})
    ...
    return jsonify({'success': True, 'message': f'字段 {field_name} 已删'})
...
def reorder_task():
    ...
    if storage.reorder_task(task_order, new_order):
        return jsonify({'success': True, 'message': '任务重排序成'})
    return jsonify({'success': False, 'message': '任务重排序失'})
...
def method_not_allowed(error):
    return jsonify({'success': False, 'message': '请求方法不允'}), 405

@app.errorhandler(500)
def internal_error(error):
    return jsonify({'success': False, 'message': '服务器内部错'}), 500
...
def reload_scheduler():
    ...
    try:
        ...
        logger.error(f'重新加载调度器失: {str(e)}')
        return jsonify({
            'success': False,
            'message': f'重新加载调度器失: {str(e)}'
        }), 500
...
def batch_delete_tasks():
    ...
    try:
        ...
        if deleted_count > 0:
            return jsonify({
                'success': True,
                'message': f'成功删除{deleted_count}个任'
            })
        else:
            return jsonify({
                'success': False,
...
def api_check_auth():
    """检查认证状"""
    if 'username' not in session:
        return jsonify({'success': False, 'message': '未登'}), 401
    ...
    if time.time() - session.get('login_time', 0) > session_timeout:
        session.clear()
        return jsonify({'success': False, 'message': '会话已过'}), 401
...
def update_auth():
    ...
    if not new_username or not new_password or not old_password:
        return jsonify({'success': False, 'message': '用户名、新密码和旧密码都不能为'})
    
    # 验证旧密
    auth_config = storage.config.get('auth', {})
    if old_password != auth_config.get('password'):
        return jsonify({'success': False, 'message': '旧密码错'})
...
def check_version():
    ...
    try:
        ...
        if source == 'dockerhub':
            ...
            try:
                params = {
                    "repositories": "WHaoKong/network-autosave",
                    "page": 1,
                    "page_size": 10,
                    "search": ""
                }
                ...
                if data.get('code') == 0 and data.get('data', {}).get('list'):
                    ...
                    # 如果没有找到与latest相同digest的版本标签，则收集所有版本标
                    if not version_tags:
                        for tag_info in data['data']['list']:
                    ...
                    if version_tags:
                        ...
                        link = f"https://hub.docker.com/layers/WHaoKong/network-autosave/{latest_version}/images/{version_tags[0].get('digest', '').split(':')[-1]}"
                        
                        logger.info(f"1ms.run API获取到最新版: {latest_version}")
                        return jsonify({
                            'success': True,
                ...
                logger.warning(f"1ms.run API获取版本信息失败: {str(e)}")
                return jsonify({
                    'success': False,
                    'message': f'1ms.run API获取版本信息失败: {str(e)}',
                    'source': '1ms'
                })
        else:
        ...
        if source in ['github', 'dockerhub', 'dockerhub_alt']:
            try:
                ...
                response.raise_for_status()  # 如果响应状态码不是200，抛出异
            ...
            if not feed.entries:
                logger.warning(f"{source}未找到版本信")
                return jsonify({
                    'success': False,
                    'message': f'{source}未找到版本信',
                    'source': source
                })
            ...
            if source in ['dockerhub', 'dockerhub_alt']:
                ...
                if not latest_entry:
                    ...
                    # 如果没有找到latest标签，使用第一个条
                    latest_entry = feed.entries[0]
                
                # 如果找到了latest的guid，查找对应的版本号条
                if latest_guid:
                    for entry in feed.entries:
                ...
                logger.info(f"从Docker Hub ({source})获取到最新版: {latest_version}")
                return jsonify({
                    'success': True,
                    'version': latest_version,
                    'published': pub_date,
                ...
                logger.info(f"从GitHub获取到最新版: {title} -> 提取版本: {latest_version}")
                return jsonify({
                    'success': True,
                    'version': latest_version,
                    'published': pub_date,
        ...
        logger.error(f"检查版本失: {str(e)}")
        return jsonify({
            'success': False,
            'message': f'检查版本失: {str(e)}',
    ...
    try:
        ...
        for task_order in tasks_to_remove:
            if task_order in app.task_logs:
                ...
                logger.debug(f"已清理任务{task_order}的历史日")
...
def share_task():
    ...
    data = request.get_json()
    task_id = data.get('task_id')
    custom_password = data.get('password')  # 可选的自定义密
    ...
    if 0 <= task_id < len(tasks):
        task = tasks[task_id]
        if task.get('provider', 'baidu') == 'quark':
            return jsonify({'success': False, 'message': 'Quark tasks do not support generating share links yet'})
...
def update_share_config():
    ...
    return jsonify({'success': True, 'message': '分享配置已更'})
...
if __name__ == '__main__':
    try:
        ...
        if not init_success:
            logger.error(f"应用初始化失: {init_error}")
            if init_error:
                logger.warning("将继续启 Web 界面，但部分功能可能不可")
